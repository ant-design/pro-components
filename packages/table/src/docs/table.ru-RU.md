...
valueEnum необходимо передать в перечислении, ProTable автоматически получит перечисление ответа на основе значения и сгенерирует раскрывающийся список из. Это выглядит так：
