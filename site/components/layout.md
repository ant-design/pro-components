...
| menuContentRender          | 包裹整块菜单区域（可改背景、加遮罩等）                                                                                                                          | `WithFalse<(props: SiderMenuProps, defaultDom: ReactNode) => ReactNode>`                                                                       | -                                        | -    |
...
| layout                     | layout 的菜单模式，side：侧栏导航，top：顶部导航；历史 `mix` 会按 `side` 处理                                                                                   | `'side'` \| `'top'`                                                                                                                            | `'side'`                                 | -    |
...
| selectedKeys               | 受控：当前选中的菜单项 key                                                                                                                                      | `string[]`                                                                                                                                     | -                                        | -    |
| openKeys                   | 受控：当前展开的子菜单 key；传 `false` 为非受控                                                                                                                 | `string[]` \| `false`                                                                                                                          | -                                        | -    |
| onSelect                   | 选中菜单项时回调                                                                                                                                                | `(info: ProLayoutNavMenuSelectInfo) => void`                                                                                                   | -                                        | -    |
| onOpenChange               | 子菜单展开/收起时回调                                                                                                                                           | `(openKeys: string[]) => void`                                                                                                                 | -                                        | -    |
| headerRender               | 自定义头的 render 方法                                                                                                                                          | `WithFalse<(props: ProLayoutProps & {hasSiderMenu?: boolean}, defaultDom: ReactNode) => ReactNode>`                                            | -                                        | -    |
| headerTitleRender          | 自定义头标题的方法，在 `layout="top"` 或侧栏且展示顶栏菜单时生效                                                                                                | `WithFalse<(logo: ReactNode, title: ReactNode, props: HeaderViewProps) => ReactNode>`                                                          | -                                        | -    |
| headerContentRender        | 自定义头内容的方法                                                                                                                                              | `WithFalse<(props: HeaderViewProps, defaultDom: ReactNode) => ReactNode>`                                                                      | -                                        | -    |
| headerMenuRender           | 侧栏布局下自定义顶栏横向菜单；`splitMenus` 为 true 时默认渲染一级菜单，传入函数可包一层或完全替换第二参数                                                       | `WithFalse<(props: HeaderViewProps, defaultMenu: ReactNode) => ReactNode>`                                                                     | -                                        | -    |
...
| collapsedButtonRender      | 自定义展开收起按钮的渲染                                                                                                                                        | `WithFalse<(collapsed?: boolean, defaultDom?: ReactNode) => ReactNode>`                                                                        | -                                        | -    |
...
| menuItemRender             | 自定义菜单项的 render 方法                                                                                                                                      | `(item: MenuDataItem & { isUrl?: boolean; onClick?: () => void }, defaultDom: ReactNode, menuConfig: BaseMenuProps) => ReactNode`              | -                                        | -    |
| menuTextRender             | 自定义菜单项展示文案（在图标旁）                                                                                                                                | `WithFalse<(item: MenuDataItem, defaultText: ReactNode, menuConfig: BaseMenuProps) => ReactNode>`                                              | -                                        | -    |
| subMenuItemRender          | 自定义拥有子菜单的菜单项 render                                                                                                                                 | `WithFalse<(item: MenuDataItem & { isUrl: boolean }, defaultDom: ReactNode, menuConfig: BaseMenuProps) => ReactNode>`                          | -                                        | -    |
...
| menuProps                  | 合并到自研主导航根节点 `nav` 的 DOM 属性（如 `className`、`style`、`id`、`aria-label`），不再透传 antd `Menu`                                                   | `ProLayoutNavMenuDomProps`                                                                                                                     | -                                        | -    |
...
| 参数                  | 说明                                                                                           | 类型                                                   | 默认值    | 版本 |
| --------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------ | --------- | ---- |
| autoClose             | 传 `false` 时关闭选中后自动收起子菜单等行为（类型为可选 `false`）                              | `false`                                                | -         | -    |
...
| request               | 远程加载菜单的方法，会自动修改 loading 状态；依赖 `params` 引用变化触发重新请求                | `(params, defaultMenuData) => Promise<MenuDataItem[]>` | -         | -    |
| params                | 与 `request` 搭配，仅当引用变化时重新请求菜单                                                  | `Record<string, any>`                                  | -         | -    |
| type                  | 菜单的类型                                                                                     | `'sub' \| 'group'`                                     | `'group'` | -    |
| collapsedShowTitle    | 收起侧栏时在图标下展示标题                                                                     | `boolean`                                              | -         | -    |
| hideMenuWhenCollapsed | 收起时是否隐藏菜单区域                                                                         | `boolean`                                              | -         | -    |
| collapsedWidth        | 侧栏收起宽度（px）                                                                             | `number`                                               | 48        | -    |
...
与根节点 `settings` 属性一致，完整类型见源码 `src/layout/defaultSettings.ts` 中的 `ProSettings`（可从 `@ant-design/pro-components` 随包导出查阅）。

常用字段如下（与根上同名 props 一并传入时以根 props 为准的常见用法，以实际合并逻辑为准）：

| 参数                       | 说明                                                                  | 类型                       |
| -------------------------- | --------------------------------------------------------------------- | -------------------------- |
| layout                     | `side` / `top`；历史 `mix` 仍出现在类型中，运行时按 `side` 处理       | `'side' \| 'top' \| 'mix'` |
| contentWidth               | 内容区 `Fluid` 或定宽 `Fixed`                                         | `'Fluid' \| 'Fixed'`       |
| fixedHeader / fixSiderbar  | 顶栏 / 侧栏是否固定                                                   | `boolean`                  |
| splitMenus                 | 顶栏展示一级菜单、侧栏展示子菜单（仅 `layout="side"` 且非移动端典型） | `boolean`                  |
| suppressSiderWhenMenuEmpty | 菜单为空时隐藏侧栏                                                    | `boolean`                  |
| siderMenuType              | 侧栏菜单形态，等同于 `menu.type` 的快捷字段                           | `'sub' \| 'group'`         |
| title                      | 站点标题；为 `false` 时标题区域仅展示页名等                           | `string \| false`          |
| colorPrimary / colorWeak   | 主色与色弱模式                                                        | `string` / `boolean`       |
| menu                       | 子项见上文 [menu](#menu)                                              | `object`                   |
| headerRender 等            | 若干 `*Render` 可设为 `false` 关闭对应区域                            | `false` 等                 |
...
在 `layout="side"` 下设置 `splitMenus=true` 可把第一级菜单放到顶栏，侧栏展示当前一级下的子菜单。也可用 `headerMenuRender` 完全自定义顶栏菜单（第二参数为默认横向菜单）。为良好体验，建议给每个一级菜单配置重定向，避免切换到白屏。
...
- icon 代表菜单图标：可为 `@ant-design/icons` 组件、自定义 React 节点，或图片/SVG 的 URL 字符串
