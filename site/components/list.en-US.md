...
| rowKey             | Row key, usually row id; falls back to `item.key` then index when unset                                                                                                                              | `keyof T` \| `(item: T) => React.Key`                                                                                                                                                                        | -                      |
...
| -------------- | -------------------------------------------------------------- | ------------------------------------ |
| reload         | Reload the list, pass true to reset page index                 | `(resetPageIndex?: boolean) => Promise<void>` |
| reloadAndRest  | Reload and clear, page index is also reset, excluding the form | `() => Promise<void>`                         |
...
| valueEnum      | Enum of values, automatically converts the value as a key to display the corresponding content       | [valueEnum](/en-US/components/schema-form#valueenum)                                     | -        |
| search         | Configure search for the column, false to hide                                                       | `boolean` \| `{ transform: (value: any) => any }`                                        | -        |
| editable       | Whether editable in editable list                                                                    | `boolean` \| `(text: any, record: T, index: number) => boolean`                          | -        |
...
> Only `type`, `title`, `subTitle`, `description`, `avatar`, `content` and `actions` keys are supported in `metas`. The `Metas.extra` documented previously does not exist in the type definitions; for aside content use `listSlot: 'aside'` in `columns` (mapped internally to the list item's `extra` area).
