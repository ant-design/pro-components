...
| itemHeaderRender | 自定义每一列的 header，与 itemRender 不同的是，它会保留多选和展开收起 | `(item: any, index: number) => ReactNode` | - |
| itemTitleRender | 自定义每一列的 title 渲染 | `(item: any, index: number, defaultDom: JSX.Element) => ReactNode` | - |
