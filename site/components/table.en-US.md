...
| tableViewRender    | Render table view, used for customizing ProList, not recommended for direct use; `defaultDom` may be a lazy factory — call when the default Table is needed (see `resolveTableViewDefaultDom`) | `(props: TableProps<DataSource>, defaultDom: JSX.Element \| (() => JSX.Element)) => JSX.Element \| undefined`                                                                                                                                              | -                                                                   |
...
| ellipsis                               | Whether to abbreviate automatically. `true` uses antd Table native CSS ellipsis (better performance, no hover tooltip); pass `{showTitle?: boolean, tooltip?: TooltipProps}` for Typography rendering with tooltip | `boolean` \| `{showTitle?: boolean}`                                                                  | -             |
